...
const findProductsByType = async ({ limit, sort, page, filter, select }) => {
  const skip = (page - 1) * limit;
  const sortBy = sort === "ctime" ? { _id: -1 } : { _id: 1 };
  const products = await product
    .find(filter)
...
const updateProductById = async ({
  ...
  const { product_review } = bodyUpdate;
  const updateOrInsert = {
    $addToSet: product_review
      ? {
  ...
  return await model.findByIdAndUpdate(productId, updateOrInsert, {
    new: isNew,
  });
};
...
const findProductById = async (productId) => {
  return await product.findOne({
    _id: convertIdToObjectMongodb(productId),
  });
};
